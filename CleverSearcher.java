...
public class CleverSearcher extends Searcher{
    ...
    @Override
    public int findElement() throws IndexingError { //Creates the findElement() method defined in "Searcher"
        int[] array = getArray(); //This makes sure that "array" contains the actual array designated in the constructor.
        ...
        int helperArray[] = new int[k]; //Creates a new array called "helperArray" which currently contains the memory needed to store k numbers but is designed to take integers.
        ...
        for (; index < array.length; index++){ //For as long as index is less than the length of the big array, run the code inside.
            if (array[index] > helperArray[0]){ //If the current number in the big array is bigger than the SMALLEST number in the small array...
